...
class com.brainoff.RegExp {
	...
	public function compile() {
		...
		var char;
		for (i=i; i<l; ++i) {
			var thischar = re.substring( i+1, 1);
			if (thischar == "\\") {
				i++;
				char = false;
				thischar = re.substring( i+1, 1);
			} else {
				char = true;
			}
			...
			if (char) {
				...
				if (thischar == "(") {
					...
					thischar = "";
					range = false;
					while (i<l && (s=re.substring( 1+i++, 1)) != "]") {
						if (range) {
							//Previous char was "-", so create a range
							if (s == "\\") {
							}
							cb = s == "\\" ? (s == "b" ? chr(8) : re.substring( 1+i++, 1)) : s;
							ca = ord(thischar.substring( thischar.length, 1))+1;
							while (cb>=(s=chr(ca++))) {
								thischar += s;
							}
							range = false;
			...
			if (nextchar == "*") {
				...
				range = "";
				i++;
				while (i+1<l && (s=re.substring( 2+i++, 1)) != "}") {
					if (!comma && s == ",") {
						comma = true;
						rangeA = Number(range);
		...
		if (!root) {
			RegExp._xiStatic = i;
			this.source = re.substring( arguments[2]+1, i-arguments[2]);
		}
		if (RegExp.d) {
			for (var ii = 0; ii<qc; ii++) {
				trace("xr"+this._xr+' '+q[ii].t+" : "+q[ii].a+" : "+q[ii].b+" : "+q[ii].s);
			}
		}
		this._xq = q;
		this._xqc = qc;
	...
	public function test() {
					...
					if (q[i].c == -10) {
						...
						if (q[i].t == 0) {
							...
							if (q[i].s == "--wb--") {
								q[i].a = 1;
								if (ix>0 && ix<l) {
									ct = str.substring( ix, 1);
									if (ct == " " || ct == "\\\\\\\\\\\\\\\\n") {
										m = 1;
									}
									if (m == 0) {
										ct = str.substring( 1+ix, 1);
										if (ct == " " || ct == "\\\\\\\\\\\\\\\\n") {
											m = 1;
										}
									}
		...
		if (r.i<0) {
			...
			ix = r.li;
			this._xi = r.i;
			RegExp._xxlm = str_.substring( r.i+1, ix-r.i);
			RegExp._xxlc = str_.substring( 1, r.i);
			RegExp._xxrc = str_.substring( ix+1, l-ix);
			if (ix == r.i) {
				ix++;
			}
			this.lastIndex = ix;
		...
		String.prototype.replace = function() {
			...
			if (re.global) {
				...
				while (re.test(s)) {
					...
					r += s.substring( ix+1, re._xi-ix)+nrs;
...
//trace(re._xi + " " + ix + " " + RegExp.lastMatch);
					if (re._xi != ix) {
						ra[rc++] = s.substring( ix+1, re._xi-ix);
					}
					ix = re._xi+(RegExp.lastMatch).length;
					ipp = ip;
					ip = re.lastIndex;
